...
前端发送给服务器，服务器返回确认消息给前端，说明前端的消息已经可靠传输给服务器，那么此时可靠性的工作就代理给服务器

使用队列结构来存储未确认的消息

### 前端将该消息存在 vuex 中（后期考虑转为 localstorage ）

前端发送消息时候会将这条消息放入确认队列中，后端返回确认消息时候会将这条消息从确认队列中去除。前端每隔一段时间回去遍历确认队列，如果此时确认队列中还存在消息说明后端没有返回确认消息，则前端重发该消息
...
服务器每向某一客户端发送（转发）消息时候，会将该消息推入队列中，收到客户端的确认消息时候会将该消息从该队列中去除。每隔一段时间回去检索该队列，如果还存在则重新发送消息。

如果需要接收的客户端已经掉线（不在线），那么他就不会返回确认消息。这条待发送的消息就会存入 redis 的离线消息中，一旦该客户端再次上线就会将这些消息推送给他。那么此时执行上述相同逻辑，将这些离线消息推入确认消息队列，只有客户端返回确认消息时候才会将这些消息从确认消息队列中删除。
